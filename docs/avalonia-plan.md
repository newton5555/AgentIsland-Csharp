...
截至 2026-09-04，P0 的 Avalonia 最小窗口、P1 的共享 Runtime 骨架、P2 的 Codex/dsh 本地数据闭环，以及 P3 前半段的 Provider 槽位视觉状态模型已经落地；Linux 真机验证和完整报告页面仍按下述阶段推进。
...
| P2 首个真实数据闭环 | 3–5 | Codex/dsh 路径、凭据读取、Token、额度/余额，最小双槽岛 | **深入推进**：Avalonia 支持 `--live` 驱动本地快照轮询；Runtime 提供跨平台 `LocalTokenSources`（Codex/dsh 本地日志、归档与 dsh 18s 本地活动状态）；同 Agent 自动通过 `CompositeAgentSnapshotSource` 融合 Token 与 Working 状态；官方余额源已完成并通过显式 `--live-online` 接入，默认 `--live` 仍离线 |
...
- 默认开发流程不接入真实账号请求或发布流程；`--live-online` 是专门的官方 DeepSeek 余额探针，其他账号额度和发布流程仍按后续阶段推进。每次阶段交付都必须保留可运行结果和回归记录。
