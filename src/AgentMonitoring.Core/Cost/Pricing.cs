...
public static class Pricing
{
    ...
    {
        dollars = 0;
        if (!Table.TryGetValue(CanonicalModelName(model), out var price)) return false;
        dollars = (inputTokens * price.Input
                + outputTokens * price.Output
                + cacheCreationTokens * price.CacheCreation
                + cacheReadTokens * price.CacheRead) / 1_000_000.0;
        return true;
    }
