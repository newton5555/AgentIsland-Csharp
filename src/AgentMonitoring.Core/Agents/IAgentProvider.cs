...
/// Official currency balance, independent of quota windows.
public sealed record AccountBalance(string Currency, double Amount, DateTimeOffset FetchedAt);

/// Balance capability: official currency balance, independent of quota windows.
public interface IBalanceFetcher
{
    ValueTask<AccountBalance?> FetchBalanceAsync(CancellationToken ct = default);
}
...
public interface IAgentProvider : IAgentModule
{
    ...
    IBalanceFetcher? BalanceFetcher => this as IBalanceFetcher;
}
