...
public sealed class TurnAlarmWindowController
{
    ...
    private static void Banner(TriggerTool provider, ActivityMonitor.ActiveThread? thread, TurnAlarmKind? kind)
    {
        ...
        if (kind is TurnAlarmKind.QuotaExhausted quota)
        {
            ...
            title = AgentIsland.UI.Localization.L10n.TrFormat("{0} {1} reached", provider.Display(), windowName);
        ...
        {
            title = AgentIsland.UI.Localization.L10n.Tr("It's your turn");
            body = thread is { } t && !string.IsNullOrWhiteSpace(t.Label)
                ? t.Label + " — " + AgentIsland.UI.Localization.L10n.Tr("The thread finished. Come back and reply.")
                : AgentIsland.UI.Localization.L10n.Tr("The thread finished. Come back and reply.");
        }
