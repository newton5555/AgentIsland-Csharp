...
* **为什么独立立项与现代 C# 完全重写？**  
  ...
  本仓库将 Windows 客户端**彻底剥离为独立的顶级 .NET 8 C# 解决方案**，由内而外进行了彻底的企业级现代化重构，专为 Windows 平台打造极致轻量、高可靠性且具备现代插件扩展能力的桌面体验。
...
| **Google Antigravity**| ✅ 会话捕获 | ✅ 令牌统计 | ➖ (平台内置绑定) | ➖ | `%USERPROFILE%\.gemini` (IDE 与 CLI 会话) |
| **xAI Grok** | ✅ 状态感知 | ✅ 令牌统计 | ✅ 用量查询 | ➖ | `%USERPROFILE%\.grok` |
| **Cursor IDE** | ✅ 活动感知 | ✅ 聚合统计 | ✅ 额度状态 | ➖ | `%APPDATA%\Cursor\...\state.vscdb` (SQLite WAL) |

---

## 🔌 架构重构红利：现代插件化与新 Agent 极简接入

原版原型采用硬编码枚举（`enum TriggerTool`）以及遍布各子模块的巨型 `switch-case`，并且假定所有 Agent 必须具备相同的“5 小时额度 + 7 天周期”模型，导致新增或维护 Agent 极其困难。

经过本次架构重构，全面转为**能力契约驱动（Capability-Driven）**的插件化体系：
...
### 3. 新增 Agent 极简 4 步法
社区开发者接入一款全新的 AI 编程 Agent（如 Windsurf、Cline、Roo Code、豆包 MarsCode 等）仅需 4 步，核心 UI 渲染管线、60fps 弹簧动效与后台调度工作者**完全零侵入**：

```csharp
// 步骤 1：在 BuiltInAgentCatalog 注册新 Agent 描述符与其支持的能力
Module("doubao", "Doubao MarsCode", 
    AgentCapabilities.Activity | AgentCapabilities.Cost, 
    cliName: "marscode");

// 步骤 2：实现所需的能力适配器（按需实现，不强求全量接口）
public class DoubaoSessionSensor : ISessionSensor
{
    public ValueTask<IReadOnlyList<ScannedSession>> ScanSessionsAsync(...) { ... }
}

// 步骤 3：在依赖注入组合根（DI Container）中挂载该适配器
services.AddSingleton<ISessionSensor, DoubaoSessionSensor>();

// 步骤 4：为该 Agent 的日志/会话解析规则补充单元测试
```
...
* **100% 纯依赖注入（Pure DI）**：彻底铲除所有静态单例（`_instance` / `Shared`），全量托管于 `Microsoft.Extensions.DependencyInjection` 容器。
...
编译输出的便携绿色包将位于 `dist/AgentIsland-1.2.0-win-x64.zip`。
